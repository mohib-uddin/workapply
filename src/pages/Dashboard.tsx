    ...
    <div className={`${bgColor} content-stretch flex flex-col items-start p-[32px] relative rounded-[4px] shrink-0 w-full max-w-[342px]`}>
      ...
      <div className="content-stretch flex flex-col gap-[28px] items-start relative shrink-0 w-full">
        ...
        <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full">
          <p className="font-['Pavanam',sans-serif] leading-[24px] text-[#faf9f6] text-[20px] opacity-70 mb-2">Job Details</p>
        ...
        <div className="bg-[#faf9f6] relative rounded-[35px] shrink-0 w-full cursor-pointer hover:bg-white transition-colors">
...
export function Dashboard({ onNavigate }: { onNavigate?: (page: 'dashboard' | 'queue' | 'applications' | 'profile') => void }) {
  ...
  return (
    <DashboardLayout currentTab="dashboard" onNavigate={onNavigate}>
      <div className="flex-1 flex flex-col items-center px-4 sm:px-6 lg:px-8 pt-[60px] lg:pt-[100px] xl:pt-[140px] pb-[60px] relative z-10 overflow-y-auto">
        {/* Title Section */}
        ...
        {/* Job Cards */}
        <div className="w-full max-w-[1400px] flex flex-col md:flex-row gap-[30px] lg:gap-[60px] xl:gap-[133px] justify-center items-center md:items-stretch pb-10">
          {jobs.map((job, index) => (
            <JobCard
              key={index}
              match={job.match}
